...
def get_gzip_blob_data(blob: storage.Blob) -> list[str]:
  """Download and process Palo Alto Network Cortex XDR log file.

  The log file is a gzipped multiple lines of JSON, unzip and parse the log
  file and return the list of JSON str.

  Args:
    blob: storage.Blob: Palo Alto Network Cortex XDR log file.

  Returns:
      list[str]: list of JSON string.
  ...
  try:
    decompressed = gzip.decompress(blob.download_as_bytes())
    file_content = decompressed.decode("utf-8").split("\n")
    # the last item is empty after the split
    return [json.loads(data) for data in file_content if data]
  except Exception as error:
    raise RuntimeError(
        f"Could not load the log data from blob {blob.name}."
    ) from error
  ...
  for bucket_name in bucket_names:
    ...
    for blob in bucket_object.list_blobs():
      ...
      if blob_created_time >= start_time:
        blob_data = get_gzip_blob_data(blob)
        if not blob_data:
          continue
