...
from MISP.main import main, get_logs_from_misp
...
@mock.patch("MISP.main.get_env_var", return_value = "test")
@mock.patch("MISP.main.ingest")
@mock.patch("MISP.main.requests.post")
class TestMISP(unittest.TestCase):
    """Test cases to verify MISP ingestion script"""

    @mock.patch("builtins.print")
    def test_http_error(self, mocked_print, mocked_post, mocked_ingest, mocked_get_env_var):
        ...
        response.json.return_value = {"message": "Bad request", "url": "/events/restSearch"}
        ...
        expected_calls = [
            mock.call("HTTP Error: 400, Reason: {'message': 'Bad request', 'url': '/events/restSearch'}"),
            mock.call("ERROR: Unexpected error occured while fetching events from the MISP API.")
        ]
        self.assertEqual(mocked_print.mock_calls[-2:], expected_calls)

    def test_no_logs_to_ingest(self, mocked_post, mocked_ingest, mocked_get_env_var):
    ...
    def test_ingest_logs(self, mocked_post, mocked_ingest, mocked_get_env_var):
    ...
    def test_log_retrieve_time(self, mocked_post, mocked_ingest, mocked_get_env_var):
        ...
        get_logs_from_misp(api_key="test", 
        ...
        self.assertEqual(kwargs.get("json").get("timestamp"), expected_log_retrieve_time)

    def test_key_to_remove(self, mocked_post, mocked_ingest, mocked_get_env_var):
        ...
        actual_data_list = get_logs_from_misp(api_key="test", 
                                              target_server="test_server",
                                              pool_interval="100",
                                              org_name="testOrg")
        expected_data_list = [
