...
from OneLogin_User.main import get_users, main

@mock.patch("OneLogin_User.main.get_env_var")
@mock.patch("OneLogin_User.main.OAuthClientCredentialsAuth")
@mock.patch('OneLogin_User.main.ingest')
class GetUsersFromOneLogin(unittest.TestCase):
    ...
    def test_multiple_page_response(self, mock_ingest, mock_oauth, mock_env_var):
        ...
        expected_calls = [
            mock.call([{'id': 1}], 'ONELOGIN_USER_CONTEXT'),
            mock.call([{'id': 2}], 'ONELOGIN_USER_CONTEXT')
        ]
        self.assertEqual(actual_calls, expected_calls)
        self.assertEqual(mock_ingest.call_count, 2)        

    @mock.patch('OneLogin_User.main.get_users')
    ...
    def test_value_error(self, mocked_print, mock_ingest, mock_oauth, mock_env_var):
        ...
        mocked_print.assert_called_with("ERROR: Unexpected data format received while collecting OneLogin users")
    ...
    @mock.patch("OneLogin_User.main.datetime")
    @mock.patch("OneLogin_User.main.get_last_run_at", return_value="2022-01-01 10:05:15.234566")
